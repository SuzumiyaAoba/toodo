...
import { Todo, TodoStatus, WorkState, PriorityLevel } from "./todo";
...
describe("Todo", () => {
  ...
  test("should update description", () => {
    const todo = new Todo(todoId, title, TodoStatus.PENDING, WorkState.IDLE, 0, now, now, now, PriorityLevel.MEDIUM, undefined, description);
  ...
  test("should not assign to project if it's the same", () => {
    const projectId = "test-project-id";
    const todo = new Todo(todoId, title, TodoStatus.PENDING, WorkState.IDLE, 0, now, now, now, PriorityLevel.MEDIUM, projectId);
    const updatedTodo = todo.assignToProject(projectId);
  ...
  test("should not add subtask if it's already added", () => {
    const subtaskId = "subtask-id";
    const todo = new Todo(todoId, title, TodoStatus.PENDING, WorkState.IDLE, 0, now, now, now, PriorityLevel.MEDIUM, undefined, undefined, undefined, [], [], undefined, [subtaskId]);
    const updatedTodo = todo.addSubtask(subtaskId);
  ...
  test("should not set parent if it's the same", () => {
    const parentId = "parent-id";
    const todo = new Todo(todoId, title, TodoStatus.PENDING, WorkState.IDLE, 0, now, now, now, PriorityLevel.MEDIUM, undefined, undefined, undefined, [], [], parentId);
    const updatedTodo = todo.setParent(parentId);
  ...
  test("should check if all subtasks are completed", () => {
    ...
    const todo = new Todo(todoId, title, TodoStatus.PENDING, WorkState.IDLE, 0, now, now, now, PriorityLevel.MEDIUM, undefined, undefined, undefined, [], [], undefined, [subtask1Id, subtask2Id]);
  ...
  test("should check if has parent", () => {
    const parentId = "parent-id";
    const todoWithParent = new Todo(todoId, title, TodoStatus.PENDING, WorkState.IDLE, 0, now, now, now, PriorityLevel.MEDIUM, undefined, undefined, undefined, [], [], parentId);
  ...
  test("should not update due date if it's the same", () => {
    const dueDate = new Date(now.getTime() + 86400000); // Tomorrow
    const todo = new Todo(todoId, title, TodoStatus.PENDING, WorkState.IDLE, 0, now, now, now, PriorityLevel.MEDIUM, undefined, undefined, dueDate);
    const updatedTodo = todo.updateDueDate(dueDate);
  ...
  test("should check if todo is overdue", () => {
    ...
    const overdueTodo = new Todo(todoId, title, TodoStatus.PENDING, WorkState.IDLE, 0, now, now, now, PriorityLevel.MEDIUM, undefined, undefined, pastDate);
    const notOverdueTodo = new Todo(todoId, title, TodoStatus.PENDING, WorkState.IDLE, 0, now, now, now, PriorityLevel.MEDIUM, undefined, undefined, futureDate);
    const completedOverdueTodo = new Todo(todoId, title, TodoStatus.COMPLETED, WorkState.COMPLETED, 0, now, now, now, PriorityLevel.MEDIUM, undefined, undefined, pastDate);
  ...
  test("should check if todo is due soon", () => {
    ...
    const veryCloseTodo = new Todo(todoId, title, TodoStatus.PENDING, WorkState.IDLE, 0, now, now, now, PriorityLevel.MEDIUM, undefined, undefined, veryCloseDate);
    const soonTodo = new Todo(todoId, title, TodoStatus.PENDING, WorkState.IDLE, 0, now, now, now, PriorityLevel.MEDIUM, undefined, undefined, soonDate);
    const farTodo = new Todo(todoId, title, TodoStatus.PENDING, WorkState.IDLE, 0, now, now, now, PriorityLevel.MEDIUM, undefined, undefined, farDate);
    const completedSoonTodo = new Todo(todoId, title, TodoStatus.COMPLETED, WorkState.COMPLETED, 0, now, now, now, PriorityLevel.MEDIUM, undefined, undefined, soonDate);
  ...
  test("should check if todo has dependency on another todo", () => {
    ...
    const todo = new Todo(todoId, title, TodoStatus.PENDING, WorkState.IDLE, 0, now, now, now, PriorityLevel.MEDIUM, undefined, undefined, undefined, [dependencyId]);
